...
public:
    ...
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        while (!imuQueOpt.empty())
        {
            ...
            if (imuTime < currentCorrectionTime - delta_t)
            {
                ...
                // if (dt <=0){
                //     RCLCPP_WARN(get_logger(), "\033[91m%s\033[0m", "This is inside of odometryHandler 1");
                //     RCLCPP_WARN(get_logger(), "\033[31m%s\033[0m", "dt is a zero or negative value");
                //     RCLCPP_WARN(get_logger(), "\033[34mthe value of dt is %lf\033[0m", (imuTime - lastImuT_opt));
        ...
        if (!imuQueImu.empty())
        {
            ...
            for (int i = 0; i < (int)imuQueImu.size(); ++i)
            {
                ...
                // if (dt <=0){
                //     RCLCPP_WARN(get_logger(), "\033[35m%s\033[0m", "This is inside of odometryHandler 2");
                //     RCLCPP_WARN(get_logger(), "\033[31m%s\033[0m", "dt is a zero or negative value");
                //     RCLCPP_WARN(get_logger(), "\033[34mthe value of dt is %lf\033[0m", dt);
                //     return;
                // }
    ...
    {
        std::lock_guard<std::mutex> lock(mtx);
        static double lastImuTime_= 0 ;
        sensor_msgs::msg::Imu thisImu = imuConverter(*imu_raw);
        double imuTime_ = stamp2Sec(thisImu.header.stamp);

        
        if (imuTime_ == lastImuTime_){
            RCLCPP_WARN(get_logger(), "\033[93m%s\033[0m", "Same time stamp ");
            return;
        }else{
            lastImuTime_ = imuTime_;
        }
        ...
        if (dt <=0){
            RCLCPP_WARN(get_logger(), "\033[93m%s\033[0m", "This is inside of imuHandler");
            RCLCPP_WARN(get_logger(), "\033[31m%s\033[0m", "dt is a zero or negative value");
            RCLCPP_WARN(get_logger(), "\033[34mthe value of dt is %lf\033[0m", dt);
            return;
        }else{
            RCLCPP_INFO(get_logger(), "\033[32m%s\033[0m", "dt is a correct value");
        }
